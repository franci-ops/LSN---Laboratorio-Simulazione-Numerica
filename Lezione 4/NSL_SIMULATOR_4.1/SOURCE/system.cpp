...
void System :: averages(int blk){
  ...
  // TO BE FIXED IN EXERCISE 4
    ...
    double normal=(1.*_npart*_bin_size_v);  //costante di normalizzzazione

 
// Ogni bin dell'istogramma viene trattato come una grandezza statistica indipendente su cui faccio l'analisi a blocchi
      for(int bin=0; bin<_n_bins_v; bin++){       //ciclo sui bin dell'istogramma del blocco corrente
        average=_average(_index_pofv+bin);        //valor medio del bin corrente per il blocco corrente
        sum_average=_global_av(_index_pofv+bin);  //accumulatore che contiene la somma di tutte le medie dei blocchi elaborate fino a quel momento
        sum_ave2=_global_av2(_index_pofv+bin);    //accumula la somma dei quadrati delle medie dei blocchi 
        

        coutf << setw(12) << bin*_bin_size_v+0.5*_bin_size_v            //valore centrale della velocità del bin  
