  ...
  if(_restart){                                     //se restart==1, non genero nuove velocità ma leggo tutto da file
    ...
    if(cinf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        ...
        _particle(i).setpositold(0, this->pbc(_side(0)*xold, 0)); //imposto posizini precedenti (necessario x verlet)
        _particle(i).setpositold(1, this->pbc(_side(1)*yold, 1)); //i-esima particella, asse y, converte da unità ridotte a reali (moltiplicando per il lato del box)
    ...
    x=int(_rnd.Rannyu(0,3)); //estraggo casualmente posizione
    sign=int(_rnd.Rannyu(0,2));  //estraggo casualmente segno

    if(sign==0){ //assegno segno - o + alla velcità
    v=-vel_T;
    }else{
    v=vel_T;
    }
    ...
    //assegno veloctà alla direzione selezionata
    if(x==0){
      vx(i)=v;
      vy(i)=0.;
      vz(i)=0.;
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.fcc");    //ATTENZIONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    for(int i=0; i<_npart; i++){              //loop sulle particelle del sistema
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x/2., 0));        //trasformo le coordinate ridotte (=normalizzate alla dimensione del mio sistema) in coordinate reali
      _particle(i).setposition(1, this->pbc(_side(1)*y/2., 1));
      _particle(i).setposition(2, this->pbc(_side(2)*z/2., 2));        //!!!!!!ESERCIZIO 4.2: DEVO OCCUPARE SOLO META' (aggiungo /2.)
