...
void System :: Verlet(){
  double xnew, ynew, znew;      //variabili per nuove coorinate particella
  for(int i=0; i<_npart; i++){ //Calcolo la forza sulla particella i-esima, nelle 3 direzioni
    _fx(i) = this->Force(i,0);
    _fy(i) = this->Force(i,1);
    _fz(i) = this->Force(i,2);
  }
  for(int i=0; i<_npart; i++){ //Verlet integration scheme    //aggiorno tutte le posizioni
    xnew = this->pbc( 2.0 * _particle(i).getposition(0,true) - _particle(i).getposition(0,false) + _fx(i) * pow(_delta,2), 0);  //trovo la posizione nuova usando la vecchia posizione, quella attuale e la forza attuale
    ...
    _particle(i).setvelocity(0, this->pbc(xnew - _particle(i).getposition(0,false), 0)/(2.0 * _delta));   //caloclo le velocità
  ...
  _naccepted += _npart;   //ogni passo è sempre accettato
  return;
}
...
void System :: initialize_velocities(){             //inizializza la velocità delle particelle
  double xold, yold, zold;                          //variabili che servono per salvare le posizioni vecchie
  double xact, yact, zact;                          //variabili in cui metto la posizione attuale letta dal file config.xyz
  double xrev, yrev, zrev;                          //variabili in cui metto la nuova "vecchia" posizione, che poi setto con setposition
  if(_restart){                                     //se restart==1, non genero nuove velocità ma leggo tutto da file
    ifstream cinf;
    cinf.open("../RISULTATI/INPUT_250/CONFIG/config.xyz");  //imposto file da cui leggo le posizioni vecchie: sono le posizioni finali della simulazione precedente, in questo modo posso andare a ritroso
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
      ...
      for(int i=0; i<_npart; i++){
        ...
        xact=_particle(i).getposition(0,true);
        yact=_particle(i).getposition(1,true);
        zact=_particle(i).getposition(2,true);
    ...
    cinf.close();
  } else { // TO BE TEMPORARILY CHANGED IN EXERCISE 4   //genro le velocità iniziali: nel 4.2 NON devo estrarle gaussianamente
    vec vx(_npart), vy(_npart), vz(_npart);         //creo vettori velocità per ogni particella
    vec sumv(_ndim);                                //vettore per somma totale velocità (per centro di massa)
    sumv.zeros();

    //impongo che il modulo della velocità sia pari a sqrt(3T) per ciascuna particella, poi estraggo casualmente direzione e segno per ciasucna particella

    double vel_T=sqrt(3.0*_temp);;
    double x, sign, v;
    
    for(int i=0; i<_npart; i++){

    x=int(_rnd.Rannyu(0,3));
    sign=int(_rnd.Rannyu(0,2));
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //DA UTILIZZARE IN ESERCIZIO 4.1
    // for (int i=0; i<_npart; i++){                   //genero casualmente la velocità + incremento la somma per la vel del CM
    ...
    for (int i=0; i<_npart; i++){                   //rimuove moto globale del sistema
      vx(i) = vx(i) - sumv(0);
      vy(i) = vy(i) - sumv(1);
      vz(i) = vz(i) - sumv(2);
    ...
    for (int i=0; i<_npart; i++){          //rende la distribuzione coerente con T e salva le velocità
      _particle(i).setvelocity(0, vx(i)*scalef);
      _particle(i).setvelocity(1, vy(i)*scalef);
      _particle(i).setvelocity(2, vz(i)*scalef);
    }
    for (int i=0; i<_npart; i++){           //ricostruisce e salva la posizione precedente
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../RISULTATI/INPUT_250/CONFIG/conf-1.xyz"); //imposto file da cui leggo posizioni attuali: sono le posizioni vecchie della simulazione precedente: in questo modo vado a ritroso
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
    ...
    cinf >> comment;
    for(int i=0; i<_npart; i++){              //loop sulle particelle del sistema
      cinf >> particle >> x >> y >> z; // units of coordinates in conf.xyz is _side (=coordinate ridotte)  //leggo le righe singolarmente
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0));        //trasformo le coordinate ridotte (=normalizzate alla dimensione del mio sistema) in coordinate reali
      _particle(i).setposition(1, this->pbc(_side(1)*y, 1));
      _particle(i).setposition(2, this->pbc(_side(2)*z, 2));        //!!!!!!ESERCIZIO 4.2: DEVO OCCUPARE SOLO META' (aggiungo /2.)
...
if(_measure_pofv){                                  //sto calcolando l'istogramma di un singolo blocco

    for(int i = 0; i < _npart; i++){                //scorro tutte le particelle per calcolare il modulo della velocità di ciasucna particella
        ...
        if(bin>=0 and bin<_n_bins_v){               //controllo che effettivamente il bin che ottengo sia sensato
         _measurement(_index_pofv+bin)++;           //incremento di 1 unità il contenuto del bin dell'istogramma
        }
    }
...
void System :: averages(int blk){
  ...
  if (_measure_kenergy){
    ...
    average  = _average(_index_kenergy);          //media del blocco corrente
    sum_average = _global_av(_index_kenergy);     //media progressiva
    sum_ave2 = _global_av2(_index_kenergy);       //media progressiva al quadrato
    coutf << setw(12) << blk                      //stampa il numero del blocco corrente
          << setw(12) << average                  
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  ...
  // TO BE FIXED IN EXERCISE 4
    ...
    if (_measure_pofv){
    coutf.open("../OUTPUT/pofv.dat",ios::app); //ios::app
    double normal=(1.*_npart*_bin_size_v);

 

      for(int bin=0; bin<_n_bins_v; bin++){       //ciclo sui bin dell'istogramma del blocco corrente
        average=_average(_index_pofv+bin);        //media dei bin del blocco
        sum_average=_global_av(_index_pofv+bin);  //accumula tutti i blocchi precedenti per quel singolo bin
        sum_ave2=_global_av2(_index_pofv+bin);
        

        coutf << setw(12) << bin*_bin_size_v+0.5*_bin_size_v              
              << setw(12) << sum_average/(double(blk)*(normal))
              << setw(12) << this->error(sum_average, sum_ave2, blk)/normal << endl;
      }
