...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open("../../NSL_SIMULATOR_7.1/RISULTATI/INPUT_MC/CONFIG/conf-1.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = double(8./3.)*M_PI*_rho*(1/(3.0*pow((_r_cut),9))-1/(pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = double(32./3.)*M_PI*_rho*(1/(3.0*pow(_r_cut,9))-0.5/(pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../../NSL_SIMULATOR_7.1/RISULTATI/INPUT_MC/CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
if(_measure_pofv){                                  //sto calcolando l'istogramma di un singolo blocco
  ...
  if (_measure_pressure){
    if(_sim_type==1){
     _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;  //in MC non esistono velocità perchè il sistema evolve tramite configurazioni spaziali ---> uso la T impostata
    }else{
     _measurement[_index_pressure] = _rho * (2.0/3.0) * kenergy_temp + (_ptail*_npart + 48.0*virial/3.0)/_volume; //in MD le particelle possiedono velocità esplicite --> uso Kenergy per ricvare T
    }
    }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
...
void System :: averages(int blk){
 ...
 for(int bin=0; bin<_n_bins; bin++){  //ciclo sui bin

        // estremi del bin
        double bin_lower=bin*_bin_size;
        double bin_upper=(bin+1)*_bin_size;

        // centro del bin (per il grafico)
        double r=bin_lower+0.5*_bin_size;

        // volume del guscio sferico con raggio compreso tra bin upper e bin lower
        double deltaV=(4.0/3.0)*M_PI*(pow(bin_upper,3)-pow(bin_lower,3));

        // normalizzazione
        double norm=_rho*_npart*deltaV;

        average=_average(_index_gofr+bin);    //metodo a blocchi per ciascun bin: sommo +bin
        sum_average=_global_av(_index_gofr+bin);
        sum_ave2=_global_av2(_index_gofr+bin);

        coutf << setw(12) << r
              << setw(12) << sum_average/(double(blk)*norm)
              << setw(12) << this->error(sum_average,sum_ave2,blk)/(norm)
              << endl;
    }
