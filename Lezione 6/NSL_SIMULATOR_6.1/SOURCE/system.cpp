...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    sx=_particle(this->pbc(i-1)).getspin();   //pbc serve perchè essendo una catena avrebbe dei bordi reali; questa funzione mi dice che se la particella i si trova al bordo, prende la prima particella della catena come prima vicina
    dx=_particle(this->pbc(i+1)).getspin();

    //probabilità condizionata per gibbs
    p=(1)/(1+exp((-2*_beta)*(_J*(sx+dx)+_H)));
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/config.ising"); //da usare per equilibrare: legge il file ising
    // cinf.open("../RISULTATI/INPUT_20000/CONFIG/conf-1.xyz");  //da usare dopo aver equlibrato: legge le configurazioni finali dell'equilibrazione
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.ising"); //da usare per equilibrare
  // cinf.open("../RISULTATI/INPUT_20000/CONFIG/config.xyz");  //da usare dopo aver equlibrato: legge le configurazioni finali dell'equilibrazione
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  } else cerr << "PROBLEM: Unable to open INPUT file config.xyz aaaa"<< endl;
  cinf.close();
...
if(_measure_pofv){                                  //sto calcolando l'istogramma di un singolo blocco
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if(_measure_magnet){  //magnetizzazione M
    double m=0.0;

    for(int i=0; i<_npart; i++){
      m+=_particle(i).getspin();
    }
    _measurement(_index_magnet)=m/double(_npart); //magnetizzazione per particella
  }
...
// TO BE FIXED IN EXERCISE 6
  ...
  if(_measure_cv){
    _measurement(_index_cv)=(tenergy_temp*tenergy_temp);  //sto salvando il quadrato dell'energia per particella
  }
...
// TO BE FIXED IN EXERCISE 6
  ...
  if(_measure_chi){
    ...
    _measurement(_index_chi)=_beta*sum*sum/double(_npart);  //chi per particella
  }
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
    if(_H==0.0){
    if(_sim_type==3){
      coutf.open("../OUTPUT/total_energy_Gibbs_0.dat",ios::app);
    }
    else{
      coutf.open("../OUTPUT/total_energy_0.dat",ios::app);
    }
    }else{
    if(_sim_type==3){
      coutf.open("../OUTPUT/total_energy_Gibbs.dat",ios::app);
    }else
    {
  ...
  // TO BE FIXED IN EXERCISE 4
    ...
    if (_measure_pofv){
    coutf.open("../OUTPUT/pofv.dat"); //ios::dat
  ...
  // TO BE FIXED IN EXERCISE 6
    ...
    //metodo a blocchi per magnetizzazione
  ...
  if(_H==0.0){
    if(_sim_type==3){
      coutf.open("../OUTPUT/cv_Gibbs_0.dat",ios::app);
    }else{
      coutf.open("../OUTPUT/cv_0.dat",ios::app);
    }
  }else{
    if(_sim_type==3){
      coutf.open("../OUTPUT/cv_Gibbs.dat",ios::app);
    }else{
      coutf.open("../OUTPUT/cv.dat",ios::app);
    }
  }

    double e, e2, cv_block;

    e=_average(_index_tenergy); //valor medio di energia per particella
    e2=_average(_index_cv);     //valor medio del quadrato di energia per particella (in cv ho infatti salvato il quadrato del valor medio per particella)

    cv_block=_beta*_beta*(e2-e*e)*_npart; //calore specifico del blocco

    _global_av(_index_cv)+=cv_block-e2;  //sottraggo e2 perchè è quello che ho salvato in cv
  ...
  // TO BE FIXED IN EXERCISE 6
    ...
    if(_H==0.0){
      if(_sim_type==3){
        coutf.open("../OUTPUT/chi_Gibbs_0.dat",ios::app);
      }else{
      coutf.open("../OUTPUT/chi_0.dat",ios::app);}
    }else{
      if(_sim_type==3){
